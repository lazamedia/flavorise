...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white shadow-md rounded-xl p-6">
        <form action="{{ route('apps.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="mt-1 block w-full p-2 border border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring focus:ring-red-100">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center space-x-2">
                <input type="checkbox" name="is_active" value="1" id="is_active"
                       class="rounded border-gray-300 text-red-600 focus:ring-red-500" checked>
